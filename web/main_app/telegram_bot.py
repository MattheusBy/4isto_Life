...
def send_to_telegram():
    ...
    message = "От:{0}\n Телефон {1}. \n Населенный пункт: {2}. \n Изделие: {3} в количестве {4}.\n Запах мочи: {5} \n Сушка: {6} \n Чистка подушек: {7}".format(
        ...
        data['drying'],
        data['pillows'],)
    url_text = "https://api.telegram.org/bot{token}/sendMessage?chat_id={chat_id}&text={message}".format(token=TOKEN, chat_id=CHAT_ID, message=message)
    url_photo = "https://api.telegram.org/bot{token}/sendPhoto?chat_id={chat_id}".format(token=TOKEN, chat_id=CHAT_ID)
    files = [
        ('photo',
    ...
    if response_text and response_photo:
        return "Message has send successful!!"